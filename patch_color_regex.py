...
if 'pow(abs(finalColor.rgb)' not in code:
    # Use regex to find the return statement at the end of the shader and prepend the math
    pattern = r'(return\s+float4\s*\(\s*saturate\s*\(\s*finalColor\.rgb\s*\)\s*,\s*finalColor\.a\s*\)\s*;)'
    injection = r'// OS Physics: Convert to Linear scRGB for FP16 Swapchain\n    finalColor.rgb = pow(abs(finalColor.rgb), 2.2);\n\n    \1'
